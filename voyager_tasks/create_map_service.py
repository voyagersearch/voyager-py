...
class AGOLHandler(object):
    ...
    def __init__(self, portal_url, username, password, service_name):
        ...
        self.token, self.http = self.get_token(portal_url, username, password)

    def get_token(self, referrer, username, password, exp=60):
        """Generates a token."""
        query_dict = {'username': username,
                      'password': password,
                      'referer': referrer}
        query_string = urllib.urlencode(query_dict)
        url = "{0}/sharing/rest/generateToken".format(referrer)
        token = json.loads(urllib.urlopen(url + "?f=json", query_string).read())
        if "token" not in token:
            print(token['error'])
            sys.exit()
        else:
            http_prefix = "{0}/sharing/rest".format(referrer)
...
def create_service(temp_folder, map_document, server_type, server_path,
    ...
    :param server_type: the type of server (i.e. ARCGIS_SERVER)
    ...
    :param username: username for ArcGIS Online (required when server type is MY_HOSTED_SERVCIES)
    :param password: password for ArcGIS Online (required when server type is MY_HOSTED_SERVCIES)

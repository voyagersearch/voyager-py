...
def clip_mxd_layers(mxd_path, coord_sys, geo_poly):
    ...
    for layer in layers:
        ...
        if layer.isFeatureLayer:
            ...
            arcpy.management.Clip(layer.dataSource, '{} {} {} {}'.format(ext.XMin, ext.YMin, ext.XMax, ext.YMax), layer.name)
...
def clip_data(datasets,
    ...
    for ds in datasets:
        try:
            ...
            if not dsc.dataType == 'File' and not dsc.dataType == 'MapDocument':
                ...
                if not gcs_sr.name == out_sr.name:
            ...
            if dsc.dataType == 'FeatureClass':
                arcpy.analysis.Clip(ds, clip_poly, dsc.name)
            elif dsc.dataType == 'ShapeFile':
                arcpy.analysis.Clip(ds, clip_poly, dsc.name[:-4])
            elif dsc.dataType == 'FeatureDataset':
                arcpy.env.workspace = ds
                for fc in arcpy.ListFeatureClasses():
                    arcpy.analysis.Clip(fc, clip_poly, os.path.join(out_gdb, fc))
                arcpy.env.workspace = out_gdb
            elif dsc.dataType == 'RasterDataset':
                ext = clip_poly.extent
                arcpy.management.Clip(ds, '{} {} {} {}'.format(ext.XMin, ext.YMin, ext.XMax, ext.YMax), dsc.name)
                ...
                for cad_fc in arcpy.ListFeatureClasses():
                    if cad_fc.lower() == 'annotation':
                        ...
                        arcpy.analysis.Clip(cad_anno, clip_poly, os.path.join(cad_wks_name, cad_fc))
                    arcpy.analysis.Clip(cad_fc, clip_poly, os.path.join(cad_wks_name, cad_fc))
                arcpy.env.workspace = out_gdb
                ...
                if dsc.catalogPath.endswith('.kml') or dsc.catalogPath.endswith('.kmz'):
                    ...
                    for layer in arcpy.mapping.ListLayers(group_layer):
                        if layer.isFeatureLayer:
                            arcpy.analysis.Clip(layer, gcs_clip_poly, arcpy.ValidateTableName(layer))
    ...
    if zipup == True:
        ...
        zip_data(out_gdb, '{}.zip'.format(zip_name))
...
if __name__ == '__main__':
    ...
    zip_up = arcpy.GetParameter(4)
    clip_data(input_datasets, output_geodatabase, clip_extent, out_cs, zip_up)

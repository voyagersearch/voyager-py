...
feature_service_point_template = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'supportfiles', 'point_feature_service_template.lyr')
feature_service_polyline_template = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'supportfiles', 'line_feature_service_template.lyr')
feature_service_polygon_template = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'supportfiles', 'poly_feature_service_template.lyr')
...
def execute(request):
    ...
    fl = query_index.fl
    fl += ',rest_endpoint,geometry_type'
    ...
    for group in groups:
        ...
        input_items = []
        for doc in docs:
            if 'rest_endpoint' in doc:
                input_items.append((doc['id'], doc['rest_endpoint'], doc['format'], doc['location'], doc['name'], doc['geometry_type']))
            else:
                input_items.append((doc['id'], doc['path'], doc['format'], doc['location'], doc['name'], doc['geometry_type']))
...
def create_layer_file(input_items, meta_folder, show_progress=False):
    ...
    for input_item in input_items:
        try:
            ...
            service_type = input_item[2]
            location = input_item[3]
            name = input_item[4]
            geometry_type = input_item[5]
            ...
            if not os.path.exists(os.path.join(layer_folder, '{0}.layer.lyr'.format(id))):
                try:
                    ...
                    if service_type == 'application/x-arcgis-map-server':
                        ...
                        lyr = urllib.urlretrieve(rest_endpoint, os.path.join(layer_folder, '{0}.layer.lyr'.format(id)))
                    elif service_type in ('application/x-arcgis-map-server-layer', 'application/x-arcgis-feature-server-layer'):
                        if service_type == 'application/x-arcgis-map-server-layer':
                            rest_endpoint = os.path.dirname(rest_endpoint) + '?f=lyr'
                            lyr = urllib.urlretrieve(rest_endpoint, os.path.join(layer_folder, '{0}.layer.lyr'.format(id)))
                        else:
                            ...
                            template_layer.replaceDataSource(os.path.dirname(rest_endpoint), 'NONE', os.path.basename(rest_endpoint), validate=False)
                            template_layer.name = name
                            lyr = template_layer.saveACopy(os.path.join(layer_folder, '{0}.layer.lyr'.format(id)))
            ...
            if lyr:
                ...
                processed_count += 1
                status_writer.send_percent(processed_count / result_count, _('Created: {0}').format('{0}.layer.lyr'.format(id)), 'create_layer_file_map_service')
        except IOError as io_err:
            processed_count += 1
            status_writer.send_percent(processed_count / result_count, _('Skipped: {0}').format(input_item), 'create_layer_file_map_service')

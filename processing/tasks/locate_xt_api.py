...
def extract(input_items, out_type, output_dir, show_progress=False):
    ...
    try:
        ...
        import comtypes.gen.ct_LocateXT_API as lxtapi
    ...
    for src_file in input_items:
        try:
            if os.path.isfile(src_file):
                manager = CreateObject(lxtapi.LocateXT_Manager, interface=lxtapi.ILocateXT_Manager)

                if out_type == 'CSV':
                    geo_info = manager.Scan(lxtapi.lxtInputTypeFilename, src_file, lxtapi.lxtOutputTypeCSV)
                    ...
                    geo_info = geo_info.replace("utf-16", "utf-8")
